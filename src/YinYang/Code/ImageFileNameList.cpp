...
void CImageFileNameList::GetBGFileNames(XMLElement* settings) {
	//image file names
	XMLElement* ist = settings->FirstChildElement("backgrounds");
	if (ist) {
		XMLElement* img; //image element

						 //count number of images
		m_nImageFileCount = 0;
		for (img = ist->FirstChildElement("background"); img; img = img->NextSiblingElement("background"))
			++m_nImageFileCount;

...
vector<CProjectile*> g_pNinjaProjectile;
...
C3DSprite* g_pBackground4 = nullptr;
C3DSprite* g_pBackground5 = nullptr;
C3DSprite* g_pBackground6 = nullptr;
C3DSprite* g_pBackground7 = nullptr;
...
C3DSprite* g_pHowToPlaySprite = nullptr;
BOOL g_bHowTo = false;
int g_nLoseYPos = 0;
int g_nWave = 1;
...
void SetLevel(bool platforms)
{
  ...
  case 4:
	  g_sBackground = "background4.txt";
	  break;

  case 5:
	  g_sBackground = "background5.txt";
	  break;

  case 6:
	  g_sBackground = "background6.txt";
	  break;

  case 7:
	  g_sBackground = "background7.txt";
	  break;
  ...
  if (platforms)
  {
	  ...
	  l_nNewPlat = g_cRandom.number(1, 10);
	  while (l_nNewPlat == g_nPlatforms)
	  {
		  l_nNewPlat = g_cRandom.number(1, 10);
	  }

	  g_nPlatforms = l_nNewPlat;
	  ...
	  case 2:
		  g_sPlatforms = "platforms2.txt";
		  break;

	  case 3:
		  g_sPlatforms = "platforms3.txt";
		  break;

	  case 4:
		  g_sPlatforms = "platforms4.txt";
		  break;

	  case 5:
		  g_sPlatforms = "platforms5.txt";
		  break;

	  case 6:
		  g_sPlatforms = "platforms6.txt";
		  break;

	  case 7:
		  g_sPlatforms = "platforms7.txt";
		  break;

	  case 8:
		  g_sPlatforms = "platforms8.txt";
		  break;

	  case 9:
		  g_sPlatforms = "platforms9.txt";
		  break;

	  case 10:
		  g_sPlatforms = "platforms10.txt";
		  break;

	  case 11:
		  g_sPlatforms = "platforms11.txt";
		  break;

	  default: // just in case by some unholy power this gets messed up
		  g_sPlatforms = "platformsBASE.txt"; // load the empty world
	  }
  }
}
...
void LoadPlatforms(){
	...
	FILE* platfile;
	fopen_s(&platfile, g_sPlatforms.c_str(), "rt");
	if (platfile){
...
void CreateObjects(){  
  ...
  for (c = 0; c < MAXPROJ; c++)
    g_pNinjaProjectile.push_back( new CProjectile(Vector3(g_nScreenWidth - 64.0f + 20 * c, g_nScreenHeight - 50.0f, 0.0f), Vector3(0.0f, 0.0f, 0.0f), g_pProjectileSprite));

	g_pEnemyCtr--;
	g_pEnemies.push_back(new CEnemyObject(Vector3((float)g_nScreenWidth / 2.0f - 150.0f, 375.0f, g_pEnemyCtr), g_pEnemyASprite, 3, ENEMY_AIR)); //big old initializer for a single enemy
	g_pEnemyCtr--;
	g_pEnemies.push_back(new CEnemyObject(Vector3((float)g_nScreenWidth / 4.0f - 150.0f, 250.0f, g_pEnemyCtr), g_pEnemyASprite, 3, ENEMY_AIR)); //big old initializer for a single enemy
	g_pEnemyCtr--;
	g_pEnemies.push_back(new CEnemyObject(Vector3((float)g_nScreenWidth / 4.0f - 50.0f, 254.0f, g_pEnemyCtr), g_pEnemyBSprite, 3, ENEMY_GROUND)); //big old initializer for a single enemy
...
void resetGame() {
	...
	g_pNinja->setVel(Vector3(0, 0, 0));
	g_pEnemies.clear(); //clears the vector
	
	g_nNumberOfEnemies = 1;
	...
	for (int c = 0; c < MAXPROJ; c++) {
		g_pNinjaProjectile[c]->Reset(); //put projectiles back
	}
	g_nProjInd = 0; //set projectile index back to beginning
	g_pSoundManager->loop(9);
} //resetGame
...
BOOL KeyboardHandler(WPARAM keystroke){ 
  switch(keystroke){
	...
	case VK_LEFT:
	case 0x41:
		if ((g_eGameState == TITLE_SCREEN || g_eGameState == PAUSE) && !themechanged)
		{
			if (g_eTheme == YIN)
			{
				g_eTheme = ZISE;
	...
	case VK_RETURN: //Enter key?
		switch (g_eGameState) {
			...
			case PAUSE:
				resetGame();
			case TITLE_SCREEN:
				g_eGameState = HOW_TO_PLAY;
				break;
			case HOW_TO_PLAY:
				if (!g_bHowTo) g_bHowTo = TRUE;
				else {
					resetGame();
					g_eGameState = GAME;
					break;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
  switch(message){ //handle message
	...
	case WM_KEYUP:
		...
		if ((g_eGameState == TITLE_SCREEN || g_eGameState == PAUSE) && (wParam == VK_LEFT || wParam == VK_RIGHT || wParam == 0x41 || wParam == 0x44))
		{
    ...
    case WM_DESTROY: //on exit
	  ...
	  delete g_pBackground4;
	  delete g_pBackground5;
	  delete g_pBackground6;
	  delete g_pBackground7;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nShow){
  ...
  g_pHowToPlaySprite = new C3DSprite();

  g_pBackground1 = new C3DSprite();
  g_pBackground2 = new C3DSprite();
  g_pBackground3 = new C3DSprite();
  g_pBackground4 = new C3DSprite();
  g_pBackground5 = new C3DSprite();
  g_pBackground6 = new C3DSprite();
  g_pBackground7 = new C3DSprite();
  ...
  if (!g_pEnemyBSprite->Load(g_cImageFileName[42]))
  {
	  ABORT("Enemy (A) image %s not found!", g_cImageFileName[42]);
  }
  ...
  if (!g_pHowToPlaySprite->Load(g_cImageFileName[41]))
  {
	  ABORT("Title image %s not found!", g_cImageFileName[41]);
  }
  ...
  // load backgrounds
  if (!g_pBackground1->Load(g_cBackgroundFileName[1]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[1]);
  }
  if (!g_pBackground2->Load(g_cBackgroundFileName[2]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[2]);
  }
  if (!g_pBackground3->Load(g_cBackgroundFileName[3]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[3]);
  }
  if (!g_pBackground4->Load(g_cBackgroundFileName[4]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[4]);
  }
  if (!g_pBackground5->Load(g_cBackgroundFileName[5]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[5]);
  }
  if (!g_pBackground6->Load(g_cBackgroundFileName[6]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[6]);
  }
  if (!g_pBackground7->Load(g_cBackgroundFileName[7]))
  {
	  ABORT("BG image %s not found!", g_cBackgroundFileName[7]);
  }
